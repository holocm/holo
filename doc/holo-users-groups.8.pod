...
The entity IDs for users and groups are C<user:foo> and C<group:foo>,
respectively, where C<foo> is the user name or group name.

=head1 OPERATIONS

=head2 Scan

Defined users and groups will be reported like this:

    group:mygroup
        found in /usr/share/holo/users-groups/10-example.toml
            with GID: 1001

    user:myuser
        found in /usr/share/holo/users-groups/10-example.toml
        found in /usr/share/holo/users-groups/20-example.toml
            with UID: 1023, home: /var/lib/myuser, login group: mygroup, groups: audio,video, login shell: /usr/bin/zsh, comment: My Own User
