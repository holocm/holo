...
C<ssh-key:$user/$ident>. For example:

    $ holo scan ssh-key:john-doe/login

    ssh-key:john-doe/login
...
The last information line contains the output of C<ssh-keygen -l -f $file>.
If the file contains multiple public keys, there will appear one such line per
key.
