...
holo B<scan> [I<-s|--short>] [I<selector> ...]
...
packages. Another well-known plugin (available separately) is
L<holo-users-groups(8)> which provisions user accounts and groups to the
standard L<passwd(5)> and L<group(5)> databases.
...
C</usr/share/holo/files/40-desktop/etc/sddm/sddm.conf>).

=over 4

=item B<scan> [I<-s|--short>] [I<selector> ...]

Prompt all plugins to scan for entities and list all known entities (or, if
selectors are given, all entities matching these selectors).
