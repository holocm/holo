...
`post_remove` is slightly more complicated because the
`/usr/share/holo/$PLUGIN_ID` directory is already gone, but Holo will refuse to
start up if it is missing and `plugin $PLUGIN_ID` is still in the `holorc`.
