...
This document describes B<version 2> of the Holo plugin interface.
...
Then add the following install/update/remove scripts to the plugin's package
(function names may differ depending on the package format):
    ...
    post_remove() {
        mkdir /usr/share/holo/$PLUGIN_ID
        holo apply file:/etc/holorc
        rmdir /usr/share/holo/$PLUGIN_ID
    }

C<post_remove> is slightly more complicated because the
C</usr/share/holo/$PLUGIN_ID> directory is already gone, but Holo will refuse to
start up if it is missing and C<plugin $PLUGIN_ID> is still in the C<holorc>.
...
this is the C<holo-users-groups> plugin.
